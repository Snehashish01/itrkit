...
2. `reports.ts` — `classifyDocument(pageText): DocumentKey | null` via document **signatures** ("Taxpayer Information Summary", "Annual Information Statement", "Annual Tax Statement"/"Form 26AS", "FORM NO. 16"/"Certificate under section 203", "Intimation u/s 143"/`"ITR3Form"` JSON). `parseReport(kind, pages)` runs per-type parsers (TIS, AIS, 26AS, Form 16, CPC/ITR computation) → `ParsedField[]`. The **Form 16 parser reads the full Part B computation** — never guesses missing detail. The **AIS parser** sums TCS from Part B1 detail rows, captures rent from Part B7, and the **prior-ITR parser** extracts `ScheduleCFL.TotalLossCFSummary` from a JSON export (best-effort, bounded recursive search). `parseWarnings(kind, pages)` emits deterministic caution strings — AIS Part B3 tax payments whose Financial Year is not the current FY (prior-year challan trap) and 26AS Part I overbooked/unmatched booking status.
...
- `compute.ts` — `compareRegimes(inputs)` runs `computeRegime` for both `'old'` and `'new'` (AY 2026-27 / FY 2025-26). Encodes: new slabs (nil ≤₹4L … 30% >₹24L) and age-banded old slabs; standard deduction (new ₹75k / old ₹50k); Chapter VI-A (old-regime deductions incl. 80EEA/80E/80EEB/80CCH, plus 80CCD(2) in both; 80TTA/80TTB capped by the savings interest surviving BFLA set-off); special rates (STCG 111A 20%, LTCG 112A 12.5% over ₹1.25L, other LTCG 12.5%); rebate 87A (new: total income ≤₹12L with marginal relief; old: ≤₹5L); **surcharge with marginal relief at ₹50L/₹1cr/₹2cr/₹5cr** (new regime caps at 25%); 4% cess. Also: a **house-property schedule** (`houseProperties[]` — self-occupied loss capped at −₹2L; let-out/deemed = 70% of NAV − interest), **brought-forward losses** (specified/sec 43(5) F&O set off against non-salary; speculative against speculative only; LTCG against LTCG; HP against HP), closing carry-forward reported in `TaxResult`, a rough **234B interest estimate**, and `recommendItrForm(inputs)` (ITR-3 for business/carry-forward, ITR-2 for capital gains, else ITR-1). Still an estimate — 234C and a few edge cases are simplified. Never a filing value.
...
- `evidencePacket` reads structured `document.analysis.fields` (falling back to legacy `facts` only when `fields` is empty) and now also sends `filingForm`, `chosenRegime`, and carry-forward/exempt-income **presence** (booleans only — never amounts beyond the existing candidate values). Same minimization rules hold: no source quotes, raw text, PAN/Aadhaar/OTP, or conversation history.

## Data schemas external tools/providers can rely on

- `DocumentAnalysis` (`src/documents/extract.ts`): `{ status: 'ready'|'unsupported'|'empty', kind: DocumentKey|null, pages: {pageNumber,text}[], fields: ParsedField[], facts: FactCandidate[], warnings?: string[] }`. `warnings` is optional (older stored documents lack it).
- `ParsedField` (`src/documents/reports.ts`): `{ key: FieldKey, label, value: number, page, confidence }`. `FieldKey` ∈ `grossSalary | salaryPaidCredited | exemptAllowances | standardDeduction | professionalTax | housePropertyIncome | rentReceived | grossTotalIncome | totalIncome | interestSavings | interestDeposit | dividend | deduction80C | deduction80CCD2 | totalTdsSalary | totalTcs | selfAssessmentTax | totalTaxPaid | bfSpecifiedBusinessLoss | bfSpeculativeBusinessLoss | bfLtcgLoss | bfHpLoss`. `housePropertyIncome` and the BF-loss fields may be **negative** (loss) only where allowed.
...
- `TaxInputs` (`src/tax/compute.ts`): normalised, all-INR inputs — `ageBand`, salary/other-source income, `houseProperties: HouseProperty[]` (schedule; drives net HP when non-empty, else the scalar `housePropertyIncome`), the broker-taxonomy capital-gains heads, brought-forward losses (`bfSpecifiedBusinessLoss`/`bfSpeculativeBusinessLoss`/`bfLtcgLoss`/`bfHpLoss`), Chapter VI-A deductions (incl. 80EEA/80E/80EEB/80CCH), `exemptIncome` (Schedule EI memo), taxes paid, and `filingMonth` (234B estimate). `emptyTaxInputs()` is the zero baseline.
- `TaxResult` / `TaxComparison` (`src/tax/compute.ts`): per-regime breakdown (`grossTotalIncome`, `bfLossSetoff`, `chapterVIADeduction`, `totalIncome`, `slabTax`, `specialTax`, `rebate87A`, `surcharge`, `cess`, `totalTaxLiability`, `taxesPaid`, `balance` — `>0` payable, `<0` refund; plus `bfLossCarriedForward`, `bfLossBreakdown`, `interest234BEstimate`, `exemptIncome`) and `{ old, new, recommended, saving }`.
- `Member.taxInputs` (`src/domain/filing.ts`): **optional** `Partial<TaxInputs>` — only user overrides are stored; income re-seeds from documents on load. `Member` also has **optional** filing metadata: `filingForm?` (`'ITR-1'|'ITR-2'|'ITR-3'`), `filingSection?`, `chosenRegime?`, `filedAt?`, `acknowledgementNo?`, `selfAssessmentChallan?` — all optional for backward compatibility.
...
- Deterministic parsing must **never invent values**. Tax figures are an **estimate only** (surcharge now modelled with marginal relief; 234C and a few edge cases still simplified) — never a filing value. **Never automate filing.**
